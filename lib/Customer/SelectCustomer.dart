...
class _SelectCustomerState extends State<SelectCustomer> {
  ...
  Future<List<Customer>> fetchItems(int offset, int limit) async {
    ...
    int? count = controller.text.isNotEmpty
        ? Sqflite.firstIntValue(await database.rawQuery(
            "SELECT * FROM Parties WHERE AreaCd=${widget.areaId.AreaCd} and dsc LIKE '%${controller.text}%'"))
        : Sqflite.firstIntValue(await database.rawQuery(
            "SELECT * FROM Parties where AreaCd=${widget.areaId.AreaCd}"));
    List<Map<String, dynamic>> parties = controller.text.isNotEmpty
        ...
        : await database.rawQuery(
            "SELECT * FROM Parties where AreaCd=${widget.areaId.AreaCd} LIMIT $limit OFFSET $offset");
    database.close();
  ...
  Future<bool> _invoiceData(Customer customer) async {
    final databasesPath = await getDatabasesPath();
    final path = getPath(databasesPath);
    Database database = await openDatabase(path);
    String query =
        'SELECT COUNT(*) FROM orders WHERE customerId = ${customer.id}';
                                                                    ...
                                                                    IconButton(
                                                                        onPressed:
                                                                          ...
                                                                          final alert =
                                                                            ...
                                                                            content:
                                                                                const Text(' It will create another order for this customer'),
                                                                            actions: <Widget>[
                                                                              TextButton(
                                                                                child: const Text('Yes'),
                                                                                onPressed: () {
