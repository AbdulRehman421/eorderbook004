...
import 'package:Mini_Bill/Invoices/invoice_model.dart';
...
class _MultiSelectCheckListScreenState
  ...
  Future<List<Product>> fetchItems(int offset, int limit) async {
    ...
    for (var product in products) {
      double price = 0;
      try {
        price = double.parse("${product['rp']}");
      } catch (e) {
        print(e);
      }
      Product p = Product(product['pcode'] ?? "", product['name1'], price, 0, 0,
              ...
              ? ElevatedButton(
                  onPressed: () async{
                    if (getSelectedItem().isNotEmpty) {
                     Order order= Order(areaId: widget.area.AreaCd,customerId: int.parse(widget.customer.id),date: _formatDate(DateTime.now()),invoiceNumber: generatedInvoiceId,time: generatedInvoiceId,paidAmount: 0,vat: 0,tax: 0,shippingCost: 0,sectorId: widget.sector.scCode);
                    await SaveInvoice(order.toMap(), getSelectedItem());
