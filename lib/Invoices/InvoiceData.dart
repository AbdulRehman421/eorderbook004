...
class MyData {
  ...
  setSectorArea() async{
    String query =
        "select Area.AreaCd as ac,Area._id as areaId,Area.SecCd,Parties.AreaCd, Sector.SecCd as sc,Area.AreaNm,Sector.SecNm,Sector._id as sectorId, Parties._id as pId,Parties.acno,Parties.dsc,Parties.Address,Parties.AreaCd from Parties inner join Area ON Area.AreaCd=Parties.AreaCd inner join Sector ON Sector.SecCd=Area.SecCd where pId=${customer.id}";
    ...
    try{
      customer=Customer(info[0]['dsc'].toString().replaceAll(RegExp(r'\s*\([^)]*\)'), ''), "${info[0]['acno']}", "${info[0]['Address']}", "","${info[0]['acno']}","${info[0]['AreaCd']}");
    }catch(e){
...
// Loop over the products in the original map
    for (var i = 0; i < products.length; i++) {
      ...
      var newKey = "$current - ${makeInvoice(DateTime.now(),time)} - $i - ${DateTime.now().microsecondsSinceEpoch}";
      newMap[newKey] = products[i].toMap(customer.id,entry);
    }
    print(newMap);
    return newMap;
  }
