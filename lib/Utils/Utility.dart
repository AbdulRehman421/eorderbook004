import 'dart:convert';
import 'dart:typed_data';

import 'package:flutter/widgets.dart';
import 'package:intl/intl.dart';
import 'package:shared_preferences/shared_preferences.dart';

// const IMAGE_KEY = 'IMAGE_KEY';

class ImageSharedPrefs {
  static Future<bool> saveImageToPrefs(String value, IMAGE_KEY) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return await prefs.setString(IMAGE_KEY, value);
  }

  static Future<bool> emptyPrefs(IMAGE_KEY) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return await prefs.remove(IMAGE_KEY);
  }

  static Future<String?> loadImageFromPrefs(IMAGE_KEY) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString(IMAGE_KEY);
  }

  // encodes bytes list as string
  static String base64String(Uint8List data) {
    return base64Encode(data);
  }


  // decode bytes from a string
  static imageFrom64BaseString(String base64String) {
    return Image.memory(
      base64Decode(base64String),
      fit: BoxFit.cover,
      height: 25,
      width: 100,
    );
  }


}

String makeInvoice(now,id){
  String formattedDate = DateFormat('ddMM').format(now);
  print(formattedDate); // Output: 0805
  String formattedNumber = id.toString().padLeft(3, '0');
  print(formattedNumber); // Output: 030
  return formattedDate+formattedNumber;
}
